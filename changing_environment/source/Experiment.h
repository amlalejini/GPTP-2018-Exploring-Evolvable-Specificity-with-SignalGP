#ifndef GPTP2018_EXPERIMENT_H
#define GPTP2018_EXPERIMENT_H
...
constexpr size_t RUN_ID__EXP_MAPELITES = 2;
...
/// Class to manage changing environment (w/logic 9) experiments.
class Experiment {
public:
  ...
  protected:
    ...
    emp::vector<size_t> functions_used_by_trial;           ///< By trial.
  ...
  public:
    Phenotype(size_t trial_cnt=1)
      : env_match_score_by_trial(trial_cnt),
        ...
        functions_used_by_trial(trial_cnt),
    ...
    void SetTrialCnt(size_t trial_cnt) {
      ...
      functions_used_by_trial.resize(trial_cnt);
    ...
    size_t GetMinFunctionsUsed() const { return functions_used_by_trial[min_trial]; }
...
protected:
  ...
  size_t MAP_ELITES__ENTROPY_RES;
  ...
  emp::vector<std::function<double(const Agent &)>> lexicase_fit_set; ///< Fit set for SGP lexicase selection.
...
public:
  Experiment(const L9ChgEnvConfig & config)
    : input_load_id(0), update(0), eval_trial(0), eval_time(0), env_state(0), dom_agent_id(0), 
      best_score(-32767), max_inst_entropy(0), functions_used(0)
  {
    ...
    MAP_ELITES__ENTROPY_RES = config.MAP_ELITES__ENTROPY_RES();
...
void Experiment::InitPopulation_FromAncestorFile() {
  ...
  std::cout << " -------------------------" << std::endl;
  std::cout << "Starting injections!" << std::endl;
  if (SELECTION_METHOD == SELECTION_METHOD_ID__MAPELITES)
    world->Inject(ancestor_prog, 1);    
  else
    world->Inject(ancestor_prog, POP_SIZE);
  std::cout <<"Done injecting!" << std::endl;
}

void Experiment::InitPopulation_Random() {
    ...
    for (size_t fID = 0; fID < fcnt; ++fID) {
      ...
      for (size_t iID = 0; iID < icnt; ++iID) {
        new_fun.PushInst(random->GetUInt(ancestor_prog.GetInstLib()->GetSize()),
                             random->GetInt(SGP__PROG_MAX_ARG_VAL),
                             random->GetInt(SGP__PROG_MAX_ARG_VAL),
                             random->GetInt(SGP__PROG_MAX_ARG_VAL),
                             tag_t());
...
void Experiment::Config_Run() {
  ...
  bool MAP_ELITES_MODE = SELECTION_METHOD == SELECTION_METHOD_ID__MAPELITES;
  ...
  if (MAP_ELITES_MODE) {
    ...
    world->SetFitFun([this](Agent & agent) {
      const size_t id = 0;
      agent.SetID(id);
      ...
      // // Debugging functions used (make sure it actually works)
      // std::cout << "===========================" << std::endl;
      // agent.GetGenome().PrintProgramFull();
      // for (size_t i = 0; i < TRIAL_CNT; ++i) {
      //   std::cout << "Functions used in trial " << i << ": " <<  agent_phen_cache[id].GetFunctionsUsed(i) << std::endl;
      // }
  ...
  if (TASKS_ON) {
    ...
    calc_score = [this](Agent & agent) {
      return agent_phen_cache[agent.GetID()].GetEnvMatchScore(eval_trial);
    };
  }
  ...
  switch (SELECTION_METHOD) {
    ...
    case SELECTION_METHOD_ID__LEXICASE: {
      ...
      for (size_t i = 0; i < task_set.GetSize(); ++i) {
        ...
        lexicase_fit_set.push_back([i, this](const Agent & agent) {
      ...
      lexicase_fit_set.push_back([this](const Agent & agent) {
        Phenotype & phen = agent_phen_cache[agent.GetID()];
        return phen.GetMinEnvMatchScore();
      });
  ...
  record_cur_phenotype_sig.AddAction([this](Agent & agent) {
    ...
    phen.SetFunctionsUsed(eval_trial, functions_used.size());
